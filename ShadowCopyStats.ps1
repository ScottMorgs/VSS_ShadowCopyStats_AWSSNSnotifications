# Original from https://gallery.technet.microsoft.com/scriptcenter/Get-Shadow-Copy-Statistics-79e05a57 //
# Changes include sending notifications to AWS SNS, outputting in .json format, appending CreationDate //
# to output .DateTime only, triggering the function inside the file with required switches//

# To run: Enter your AWS SNS ARN, region and file server name ie ABCFS01 as per the comments below.
...
# Enter your AWS SNS ARN here:	    
        [string]$TopicArn = 'YOURAWSSNSARN',
# Enter your region here:	
	    [string]$Region = 'ap-southeast-2',
	    [string]$Subject = "Shadow Copy Data.",
	    [string]$Message = $ShadowCopyStats,
	    [switch]$EmailLog
    )
    Begin
			    ...
			    If($Volumes)
			    {
				    ...
				    ForEach($Volume in $Volumes)
				    {
					    ...
					    {	$Object.ShadowCopyCount = (($VolumeShadowCopies | Measure-Object -Property Count -Sum).Sum)
						    $Object.OldestShadowCopy = (($VolumeShadowCopies | Select -First 1).CreationDate.DateTime)
						    $Object.LatestShadowCopy = (($VolumeShadowCopies | Select -Last 1).CreationDate.DateTime)
    ...
    {	$ErrorActionPreference = $script:CurrentErrorActionPreference
	    If($EmailLog)
	    {
            AWSSNS_SendEmail -TopicArn $TopicArn -Subject $Subject -Region $Region -Message ($ShadowCopyStats | ConvertTo-Json)
	    }
	    $ShadowCopyStats
    }
}
# Enter your filserver name here:
GetVSSInfo -ServerName YOURFILESERVERNAME -EmailLog
